...
int rtl8168_led_mod_ctrl(struct rtl8169_private *tp, u16 mask, u16 val)
{
	...
	printk("LED_DEBUG: reg=0x%x, mask=0x%04x, val=0x%04x\n",
		 LED_CTRL, mask, val);	
...
int rtl8125_set_led_mode(struct rtl8169_private *tp, int index, u16 mode)
{
	...
	printk("LED_DEBUG: index=%d, reg=0x%x, old_val=0x%04x, new_mode=0x%04x\n",
		 index, reg, val, mode);
...
static int rtl_init_one(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (rtl_is_8125(tp)){
		rtl8125_set_led_mode(tp, 0, RTL8125_LED_CTRL_LINK_100 |
						RTL8125_LED_CTRL_LINK_1000 |
						RTL8125_LED_POLARITY_CTRL);

		rtl8125_set_led_mode(tp, 1, RTL8125_LED_CTRL_ACT);
	}else if (tp->mac_version > RTL_GIGA_MAC_VER_06){
		u16 mask, val;
		...
		((RTL8168_LED_CTRL_LINK_10 |
			  ...
			  RTL8168_LED_CTRL_LINK_1000) << 4) |
			  RTL8168_LED1_POLARITY_CTRL |

			(RTL8168_LED_CTRL_ACT << 8);

		rtl8168_led_mod_ctrl(tp, mask, val);
	}
