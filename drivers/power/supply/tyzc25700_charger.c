...
#include <linux/power/tyzc25700-charge.h>
#include <linux/i2c.h>
...
#define tyzc25700_info(fmt, args...) pr_info("tyzc25700: "fmt, ##args)

#define TYZC25700_MANUFACTURER		"Texas Instruments"
#define TYZC25700_ID			0x59
#define TYZC25703_ID			0x58
...
struct tyzc25700_device {
	...
	struct tyzc2570x_platform_data	plat_data;
	...
	int				chip_id;
	struct tyzc25700_init_data	init_data;
	struct tyzc25700_state		state;
	int				pd_charge_only;
	unsigned int			bc_event;
	bool				usb_bc;
};

static const struct reg_field tyzc25700_reg_fields[] = {
	/*REG12*/
...
static const struct reg_field tyzc25703_reg_fields[] = {
	/*REG00*/
...
struct tyzc25700_range {
	u32 min;
	u32 max;
	u32 step;
};

struct tyzc25700_lookup {
	const u32 *tbl;
	u32 size;
};

static const struct tyzc25700_range sc8886_otg_range = {
	.min = 1280000,
	.max = 20800000,
	.step = 128000,
};
...
static union {
	struct tyzc25700_range  rt;
	struct tyzc25700_lookup lt;
} tyzc25700_tables[] = {
	/* range tables */
...
static const struct regmap_range tyzc25700_readonly_reg_ranges[] = {
	regmap_reg_range(0x20, 0x26),
	regmap_reg_range(0xFE, 0xFF),
};

static const struct regmap_access_table tyzc25700_writeable_regs = {
	.no_ranges = tyzc25700_readonly_reg_ranges,
	.n_no_ranges = ARRAY_SIZE(tyzc25700_readonly_reg_ranges),
};

static const struct regmap_range tyzc25700_volatile_reg_ranges[] = {
...
static const struct regmap_access_table tyzc25700_volatile_regs = {
	.yes_ranges = tyzc25700_volatile_reg_ranges,
	.n_yes_ranges = ARRAY_SIZE(tyzc25700_volatile_reg_ranges),
};

static const struct regmap_config tyzc25700_regmap_config = {
	.reg_bits = 8,
	.val_bits = 16,
	...
	.wr_table = &tyzc25700_writeable_regs,
	.volatile_table = &tyzc25700_volatile_regs,
	.val_format_endian = REGMAP_ENDIAN_LITTLE,
};

static const struct regmap_range tyzc25703_readonly_reg_ranges[] = {
	regmap_reg_range(0x20, 0x2F),
};

static const struct regmap_access_table tyzc25703_writeable_regs = {
	.no_ranges = tyzc25703_readonly_reg_ranges,
	.n_no_ranges = ARRAY_SIZE(tyzc25703_readonly_reg_ranges),
};

static const struct regmap_range tyzc25703_volatile_reg_ranges[] = {
	regmap_reg_range(0x00, 0x0F),
	regmap_reg_range(0x20, 0x3B),
};

static const struct regmap_access_table tyzc25703_volatile_regs = {
	.yes_ranges = tyzc25703_volatile_reg_ranges,
	.n_yes_ranges = ARRAY_SIZE(tyzc25703_volatile_reg_ranges),
};

static const struct regmap_config tyzc25703_regmap_config = {
	.reg_bits = 8,
	.val_bits = 16,
	...
	.wr_table = &tyzc25703_writeable_regs,
	.volatile_table = &tyzc25703_volatile_regs,
	.val_format_endian = REGMAP_ENDIAN_LITTLE,
};

static void tyzc25700_disable_charge(struct tyzc25700_device *charger);

static struct tyzc25700_device *tyzc25700_charger;

static int tyzc25700_field_read(struct tyzc25700_device *charger,
			      enum tyzc25700_fields field_id)
{
	int ret;
	int val;
...
static int tyzc25700_field_write(struct tyzc25700_device *charger,
			       enum tyzc25700_fields field_id, unsigned int val)
{
	return regmap_field_write(charger->rmap_fields[field_id], val);
}

static int tyzc25700_get_chip_state(struct tyzc25700_device *charger,
				  struct tyzc25700_state *state)
{
	int i, ret;

	struct {
		enum tyzc25700_fields id;
		u8 *data;
	} state_fields[] = {
	...
	for (i = 0; i < ARRAY_SIZE(state_fields); i++) {
		ret = tyzc25700_field_read(charger, state_fields[i].id);
		if (ret < 0)
			return ret;
...
static int tyzc25700_dump_regs(struct tyzc25700_device *charger)
{
	u32 val = 0;
	struct tyzc25700_state state;
	int ret = 0;

	ret = tyzc25700_field_write(charger, ADC_START, 1);
	if (ret < 0) {
		DBG("error: ADC_START\n");
		return ret;
	}
	...
	DBG("battery charge current: %dmA\n",
	    tyzc25700_field_read(charger, OUTPUT_DSG_CUR) * 64);
	DBG("battery discharge current: %dmA\n",
	    tyzc25700_field_read(charger, OUTPUT_CHG_CUR) * 256);
	DBG("VSYS volatge: %dmV\n",
	    2880 + tyzc25700_field_read(charger, OUTPUT_SYS_VOL) * 64);
	DBG("BAT volatge: %dmV\n",
	    2880 + tyzc25700_field_read(charger, OUTPUT_BAT_VOL) * 64);

	DBG("SET CHARGE_CURRENT: %dmA\n",
	    tyzc25700_field_read(charger, CHARGE_CURRENT) * 64);
	DBG("MAX_CHARGE_VOLTAGE: %dmV\n",
	    tyzc25700_field_read(charger, MAX_CHARGE_VOLTAGE) * 16);
	DBG("	  INPUT_VOLTAGE: %dmV\n",
	    3200 + tyzc25700_field_read(charger, INPUT_VOLTAGE) * 64);
	DBG("	  INPUT_CURRENT: %dmA\n",
	    tyzc25700_field_read(charger, INPUT_CURRENT) * 50);
	DBG("	 MIN_SYS_VOTAGE: %dmV\n",
	    1024 + tyzc25700_field_read(charger, MIN_SYS_VOTAGE) * 256);
	tyzc25700_get_chip_state(charger, &state);
	DBG("status:\n");
	DBG("AC_STAT:  %d\n", state.ac_stat);
	DBG("ICO_DONE: %d\n", state.ico_done);
	DBG("IN_VINDPM: %d\n", state.in_vindpm);
...
static int tyzc25703_dump_regs(struct tyzc25700_device *charger)
{
	int i = 0;
	u32 val = 0;
	struct tyzc25700_state state;
...
static ssize_t tyzc25700_charge_info_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct tyzc25700_device *charger = dev_get_drvdata(dev);

	if ((charger->chip_id & 0xff) == TYZC25700_ID)
		tyzc25700_dump_regs(charger);
	if ((charger->chip_id & 0xff) == TYZC25703_ID)
		tyzc25703_dump_regs(charger);

	return 0;
}

static struct device_attribute tyzc25700_charger_attr[] = {
	__ATTR(charge_info, 0664, tyzc25700_charge_info_show, NULL),
};

static void tyzc25700_init_sysfs(struct tyzc25700_device *charger)
{
	int i, ret;

	for (i = 0; i < ARRAY_SIZE(tyzc25700_charger_attr); i++) {
		ret = sysfs_create_file(&charger->dev->kobj,
					&tyzc25700_charger_attr[i].attr);
		if (ret)
			dev_err(charger->dev, "create charger node(%s) error\n",
				tyzc25700_charger_attr[i].attr.name);
	}
}

static u32 tyzc25700_find_idx(u32 value, enum tyzc25700_table_ids id)
{
	u32 idx;
	u32 rtbl_size;
	const struct tyzc25700_range *rtbl = &tyzc25700_tables[id].rt;
...
void tyzc25700_charger_set_current(unsigned long event,
				 int current_value)
{
	int idx;

	if (!tyzc25700_charger) {
		pr_err("[%s,%d] tyzc25700_charger is null\n", __func__, __LINE__);
		return;
	}
	switch (event) {
	case CHARGER_CURRENT_EVENT:
		idx = tyzc25700_find_idx(current_value, TBL_ICHG);
		tyzc25700_field_write(tyzc25700_charger, CHARGE_CURRENT, idx);
		break;

	case INPUT_CURRENT_EVENT:
		idx = tyzc25700_find_idx(current_value, TBL_INPUTCUR);
		tyzc25700_field_write(tyzc25700_charger, INPUT_CURRENT, idx);
		break;
...
static int tyzc25700_fw_read_u32_props(struct tyzc25700_device *charger)
{
	int ret;
	u32 property;
	int i;
	struct tyzc25700_init_data *init = &charger->init_data;
	struct {
		char *name;
		bool optional;
		enum tyzc25700_table_ids tbl_id;
	...
	for (i = 0; i < ARRAY_SIZE(props); i++) {
		...
		if (props[i].tbl_id == TBL_OTGVOL) {
			if (of_device_is_compatible(charger->dev->of_node,
						    "southchip,sc8886")) {
				tyzc25700_tables[TBL_OTGVOL].rt = sc8886_otg_range;
		...
		*props[i].conv_data = tyzc25700_find_idx(property,
						       props[i].tbl_id);
...
static int tyzc25700_hw_init(struct tyzc25700_device *charger)
{
	int ret;
	int i;
	struct tyzc25700_state state;
	...
	/* disable watchdog */
	ret = tyzc25700_field_write(charger, WDTWR_ADJ, 0);
	if (ret < 0)
		return ret;

	/* initialize currents/voltages and other parameters */
	for (i = 0; i < ARRAY_SIZE(init_data); i++) {
		ret = tyzc25700_field_write(charger, init_data[i].id,
					  init_data[i].value);
		if (ret < 0)
			return ret;
	}
	...
	ret = tyzc25700_field_write(charger, EN_LWPWR, 0);
	if (ret < 0) {
		DBG("error: EN_LWPWR\n");
		return ret;
	}

	ret = tyzc25700_field_write(charger, ADC_CONV, 1);
	if (ret < 0) {
		DBG("error: ADC_CONV\n");
		return ret;
	}
	...
	ret = tyzc25700_field_write(charger, ADC_FULLSCALE, 1);
	if (ret < 0) {
		DBG("error: ADC_FULLSCALE\n");
		return ret;
	}

	ret = tyzc25700_field_write(charger, EN_ADC_CMPIN, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_CMPIN\n");
		return ret;
	}

	ret = tyzc25700_field_write(charger, EN_ADC_VBUS, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_VBUS\n");
		return ret;
	}

	ret = tyzc25700_field_write(charger, EN_ADC_PSYS, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_PSYS\n");
		return ret;
	}

	ret = tyzc25700_field_write(charger, EN_ADC_IIN, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_IIN\n");
		return ret;
	}
	
	ret = tyzc25700_field_write(charger, EN_OTG, 1);
	if (ret < 0) {
		DBG("error: EN_OTG\n");
		return ret;
	}	

	ret = tyzc25700_field_write(charger, EN_ADC_IDCHG, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_IDCHG\n");
		return ret;
	}

	ret = tyzc25700_field_write(charger, EN_ADC_ICHG, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_ICHG\n");
		return ret;
	}

	ret = tyzc25700_field_write(charger, EN_ADC_VSYS, 1);
	if (ret < 0) {
		DBG("error: EN_ADC_VSYS\n");
		return ret;
	}

	ret = tyzc25700_field_write(charger, EN_ADC_VBAT, 1);
	if (ret < 0) {
...
static int tyzc25700_fw_probe(struct tyzc25700_device *charger)
{
	int ret;

	ret = tyzc25700_fw_read_u32_props(charger);
	if (ret < 0)
		return ret;
...
static void tyzc25700_enable_charger(struct tyzc25700_device *charger,
				   u32 input_current)
{
	tyzc25700_field_write(charger, INPUT_CURRENT, input_current);
	tyzc25700_field_write(charger, CHARGE_CURRENT, charger->init_data.ichg);
}

static enum power_supply_property tyzc25700_power_supply_props[] = {
...
static int tyzc25700_power_supply_get_property(struct power_supply *psy,
					     enum power_supply_property psp,
					     union power_supply_propval *val)
{
	int ret;
	struct tyzc25700_device *bq = power_supply_get_drvdata(psy);
	struct tyzc25700_state state;
	...
	case POWER_SUPPLY_PROP_MANUFACTURER:
		val->strval = TYZC25700_MANUFACTURER;
		break;
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT:
		/* read measured value */
		ret = tyzc25700_field_read(bq, OUTPUT_CHG_CUR);
		if (ret < 0)
			return ret;
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		val->intval = tyzc25700_tables[TBL_ICHG].rt.max;
		break;
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE:
		...
		/* read measured value */
		ret = tyzc25700_field_read(bq, OUTPUT_BAT_VOL);
		if (ret < 0)
			return ret;
	...
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_VOLTAGE_MAX:
		val->intval = tyzc25700_tables[TBL_CHGMAX].rt.max;
		break;

	case POWER_SUPPLY_PROP_CHARGE_CONTROL_LIMIT_MAX:
		val->intval = tyzc25700_tables[TBL_INPUTVOL].rt.max;
		break;

	case POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT:
		val->intval = tyzc25700_tables[TBL_INPUTCUR].rt.max;
		break;

	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		ret = tyzc25700_field_read(bq, MAX_CHARGE_VOLTAGE);
		val->intval = ret * 16;
		break;

	case POWER_SUPPLY_PROP_CURRENT_MAX:
		ret = tyzc25700_field_read(bq, CHARGE_CURRENT);
		val->intval = ret * 64;
		break;
...
static char *tyzc25700_charger_supplied_to[] = {
	"charger",
};

static const struct power_supply_desc tyzc25700_power_supply_desc = {
	.name = "tyzc25700-charger",
	.type = POWER_SUPPLY_TYPE_USB,
	.properties = tyzc25700_power_supply_props,
	.num_properties = ARRAY_SIZE(tyzc25700_power_supply_props),
	.get_property = tyzc25700_power_supply_get_property,
};

static int tyzc25700_power_supply_init(struct tyzc25700_device *charger)
{
	struct power_supply_config psy_cfg = { .drv_data = charger, };

	psy_cfg.supplied_to = tyzc25700_charger_supplied_to;
	psy_cfg.num_supplicants = ARRAY_SIZE(tyzc25700_charger_supplied_to);
	psy_cfg.of_node = charger->dev->of_node;

	charger->supply_charger =
		power_supply_register(charger->dev,
				      &tyzc25700_power_supply_desc,
				      &psy_cfg);

	return PTR_ERR_OR_ZERO(charger->supply_charger);
}

static void tyzc25700_discnt(struct tyzc25700_device *charger, enum tpyec_port_t port);

static int tyzc2570x_pd_notifier_call(struct notifier_block *nb,
		unsigned long val, void *v)
{
	struct tyzc25700_device *bq =
		container_of(nb, struct tyzc25700_device, nb);
	...
	if (ret != 0)
		return NOTIFY_OK;
	if (prop.intval > 0) {
		cur_idx = tyzc25700_find_idx(prop.intval, TBL_INPUTCUR);
		ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_VOLTAGE_NOW,
						&prop);
		if (ret != 0)
			return NOTIFY_OK;
		vol_idx = tyzc25700_find_idx((prop.intval - 1280000 - 3200000), TBL_INPUTVOL);
		ret = power_supply_get_property(psy, POWER_SUPPLY_PROP_CURRENT_NOW,
						&prop);
		if (ret != 0)
			return NOTIFY_OK;
		chr_idx = tyzc25700_find_idx(prop.intval, TBL_ICHG);

		tyzc25700_field_write(bq, INPUT_CURRENT, cur_idx);
		tyzc25700_field_write(bq, INPUT_VOLTAGE, vol_idx);
		tyzc25700_field_write(bq, CHARGE_CURRENT, chr_idx);
		dev_info(bq->dev, "INPUT_CURRENT:%d, INPUT_VOLTAGE:%d, CHARGE_CURRENT:%d\n",
			 cur_idx, vol_idx, chr_idx);

		tyzc25700_get_chip_state(bq, &state);
		bq->state = state;
...
static irqreturn_t tyzc25700_irq_handler_thread(int irq, void *private)
{
	struct tyzc25700_device *charger = private;
	struct tyzc25700_state state;

	//printk("----jkren----tyzc25700_irq_handler_thread--------\n");
	if (!tyzc25700_field_read(charger, AC_STAT)) {
		tyzc25700_field_write(charger, INPUT_CURRENT,
				    charger->init_data.input_current_sdp);
		tyzc25700_disable_charge(charger);
		tyzc25700_get_chip_state(charger, &state);
		charger->state = state;
...
static void tyzc25700_enable_typec0(struct tyzc25700_device *charger)
{
...
static void tyzc25700_enable_typec1(struct tyzc25700_device *charger)
{
	if (!IS_ERR_OR_NULL(charger->typec0_enable_io))
...
static void tyzc25700_typec0_discharge(struct tyzc25700_device *charger)
{
...
static void tyzc25700_typec1_discharge(struct tyzc25700_device *charger)
{
...
static void tyzc25700_charger_evt_handel(struct tyzc25700_device *charger,
				       ...
				       enum tpyec_port_t port)
{
	...
	if (extcon_get_state(edev, EXTCON_CHG_USB_SDP) > 0) {
		...
		tyzc25700_enable_charger(charger,
				       charger->init_data.input_current_sdp);
...
static void tyzc25700_charger_usb_bc_handel(struct tyzc25700_device *charger)
{
	struct tyzc25700_state state;
...
static void tyzc25700_charger_evt_worker(struct work_struct *work)
{
	struct tyzc25700_device *charger = container_of(work,
				struct tyzc25700_device, usb_work.work);
	struct extcon_dev *edev = charger->cable_edev;

	if (charger->usb_bc == 0)
		tyzc25700_charger_evt_handel(charger, edev, USB_TYPEC_0);
	else
		tyzc25700_charger_usb_bc_handel(charger);
}

static void tyzc25700_charger_evt_worker1(struct work_struct *work)
{
	struct tyzc25700_device *charger = container_of(work,
				struct tyzc25700_device, usb_work1.work);
	struct extcon_dev *edev = charger->cable_edev_1;

	tyzc25700_charger_evt_handel(charger, edev, USB_TYPEC_1);
}

static int tyzc25700_charger_evt_notifier(struct notifier_block *nb,
					unsigned long event,
					void *ptr)
{
	struct tyzc25700_device *charger =
		container_of(nb, struct tyzc25700_device, cable_cg_nb);
...
static int tyzc25700_charger_evt_notifier1(struct notifier_block *nb,
					 unsigned long event,
					 void *ptr)
{
	struct tyzc25700_device *charger =
		container_of(nb, struct tyzc25700_device, cable_cg_nb1);
...
static void tyzc25700_set_otg_vbus(struct tyzc25700_device *charger, bool enable)
{
	DBG("OTG %s\n", enable ? "enable" : "disable");

	if (!IS_ERR_OR_NULL(charger->otg_mode_en_io))
		gpiod_direction_output(charger->otg_mode_en_io, enable);
	tyzc25700_field_write(charger, EN_OTG, enable);
}

static void tyzc25700_host_evt_worker(struct work_struct *work)
{
	struct tyzc25700_device *charger =
		container_of(work, struct tyzc25700_device, host_work.work);
	struct extcon_dev *edev = charger->cable_edev;

	if (extcon_get_state(edev, EXTCON_USB_VBUS_EN) > 0)
		tyzc25700_set_otg_vbus(charger, true);
	else if (extcon_get_state(edev, EXTCON_USB_VBUS_EN) == 0)
		tyzc25700_set_otg_vbus(charger, false);
}

static void tyzc25700_host_evt_worker1(struct work_struct *work)
{
	struct tyzc25700_device *charger =
		container_of(work, struct tyzc25700_device, host_work1.work);
	struct extcon_dev *edev = charger->cable_edev_1;
...
static int tyzc25700_host_evt_notifier(struct notifier_block *nb,
				     unsigned long event, void *ptr)
{
	struct tyzc25700_device *charger =
		container_of(nb, struct tyzc25700_device, cable_host_nb);
...
static int tyzc25700_host_evt_notifier1(struct notifier_block *nb,
				      unsigned long event, void *ptr)
{
	struct tyzc25700_device *charger =
		container_of(nb, struct tyzc25700_device, cable_host_nb1);
...
static void tyzc25700_discnt(struct tyzc25700_device *charger,
			   enum tpyec_port_t port)
{
	int vol_idx;
	struct tyzc25700_state state;

	if (tyzc25700_field_read(charger, AC_STAT) == 0) {
		tyzc25700_disable_charge(charger);
		if (port == USB_TYPEC_0) {
			tyzc25700_typec0_discharge(charger);
			charger->typec0_status = USB_STATUS_NONE;
		} else {
			tyzc25700_typec1_discharge(charger);
			charger->typec1_status = USB_STATUS_NONE;
		}

		vol_idx = tyzc25700_find_idx(DEFAULT_INPUTVOL, TBL_INPUTVOL);
		tyzc25700_field_write(charger, INPUT_VOLTAGE, vol_idx);
		tyzc25700_field_write(charger, INPUT_CURRENT,
				    charger->init_data.input_current_sdp);
		tyzc25700_get_chip_state(charger, &state);
		charger->state = state;
...
static void tyzc25700_discnt_evt_worker(struct work_struct *work)
{
	struct tyzc25700_device *charger = container_of(work,
						      struct tyzc25700_device,
						      discnt_work.work);

	tyzc25700_discnt(charger, USB_TYPEC_0);
}

static int tyzc25700_register_cg_extcon(struct tyzc25700_device *charger,
				      struct extcon_dev *edev,
...
static int tyzc25700_register_cg_nb(struct tyzc25700_device *charger)
{
	...
	if (charger->usb_bc == 0) {
		if (charger->cable_edev) {
			...
			INIT_DELAYED_WORK(&charger->usb_work,
					  tyzc25700_charger_evt_worker);
			charger->cable_cg_nb.notifier_call =
				tyzc25700_charger_evt_notifier;
			tyzc25700_register_cg_extcon(charger, charger->cable_edev,
						   &charger->cable_cg_nb);
		}

		if (charger->cable_edev_1) {
			INIT_DELAYED_WORK(&charger->usb_work1,
					  tyzc25700_charger_evt_worker1);
			charger->cable_cg_nb1.notifier_call =
				tyzc25700_charger_evt_notifier1;
			tyzc25700_register_cg_extcon(charger,
...
static int tyzc25700_register_pd_nb(struct tyzc25700_device *charger)
{
	...
	if (charger->notify_node || charger->plat_data.notify_device) {
		INIT_DELAYED_WORK(&charger->discnt_work,
				  tyzc25700_discnt_evt_worker);
		charger->nb.notifier_call = tyzc2570x_pd_notifier_call;
	...
	if (charger->nb.notifier_call) {
		if (charger->dev->of_node) {
			...
			if (IS_ERR_OR_NULL(notify_psy)) {
				dev_info(charger->dev, "tyzc25700 notify_psy is error\n");
				notify_psy = NULL;
			}
	...
	if (notify_psy) {
		...
		cur_idx = tyzc25700_find_idx(prop.intval, TBL_INPUTCUR);
		vol_idx = tyzc25700_find_idx((prop.intval - 1280000 - 3200000), TBL_INPUTVOL);
		tyzc25700_field_write(charger, INPUT_CURRENT, cur_idx);
		tyzc25700_field_write(charger, INPUT_VOLTAGE, vol_idx);
		tyzc25700_field_write(charger, CHARGE_CURRENT,
...
static int tyzc25700_register_host_nb(struct tyzc25700_device *charger)
{
	int ret;

	/* Register host */
	if (charger->cable_edev) {
		INIT_DELAYED_WORK(&charger->host_work, tyzc25700_host_evt_worker);
		charger->cable_host_nb.notifier_call =
			tyzc25700_host_evt_notifier;
	...
	if (charger->cable_edev_1) {
		INIT_DELAYED_WORK(&charger->host_work1,
				  tyzc25700_host_evt_worker1);
		charger->cable_host_nb1.notifier_call =
			tyzc25700_host_evt_notifier1;
...
static int tyzc25700_otg_vbus_enable(struct regulator_dev *dev)
{
	struct tyzc25700_device *charger = rdev_get_drvdata(dev);
...
static int tyzc25700_otg_vbus_disable(struct regulator_dev *dev)
{
	struct tyzc25700_device *charger = rdev_get_drvdata(dev);
...
static int tyzc25700_otg_vbus_is_enabled(struct regulator_dev *dev)
{
	struct tyzc25700_device *charger = rdev_get_drvdata(dev);
	u8 val;
	int gpio_status = 1;

	val = tyzc25700_field_read(charger, EN_OTG);
	if (!IS_ERR_OR_NULL(charger->otg_mode_en_io))
...
static const struct regulator_ops tyzc25700_otg_vbus_ops = {
	.enable = tyzc25700_otg_vbus_enable,
	.disable = tyzc25700_otg_vbus_disable,
	.is_enabled = tyzc25700_otg_vbus_is_enabled,
};

static const struct regulator_desc tyzc25700_otg_vbus_desc = {
	.name = "otg-vbus",
	.of_match = "otg-vbus",
	.regulators_node = of_match_ptr("regulators"),
	.owner = THIS_MODULE,
	.ops = &tyzc25700_otg_vbus_ops,
	.type = REGULATOR_VOLTAGE,
	.fixed_uV = 5000000,
	.n_voltages = 1,
};

static int tyzc25700_register_otg_vbus_regulator(struct tyzc25700_device *charger)
{
...
static long tyzc25700_init_usb(struct tyzc25700_device *charger)
{
	...
	if (!charger->pd_charge_only)
		tyzc25700_register_cg_nb(charger);

	if (tyzc25700_register_otg_vbus_regulator(charger) < 0) {
		dev_warn(charger->dev,
			 "Cannot register otg vbus regulator\n");
		charger->otg_vbus_reg = NULL;
		tyzc25700_register_host_nb(charger);
	}

	tyzc25700_register_pd_nb(charger);
...
static int tyzc25700_parse_dt(struct tyzc25700_device *charger)
{
	int ret;
...
#if 0
void update_charger_status(void)
{
	schedule_delayed_work(&mycharger->usb_work, 0);
}

int get_charger_status(void)
{	
	return mycharger->state.ac_stat ;
}
#endif
static int tyzc25700_probe(struct i2c_client *client,
			 const struct i2c_device_id *id)
{
	...
	charger_np = of_find_compatible_node(NULL, NULL, "ti,tyzc25700");
	if (!charger_np)
		charger_np = of_find_compatible_node(NULL, NULL, "southchip,sc8885");
	if (charger_np) {
		charger->regmap = devm_regmap_init_i2c(client,
						       &tyzc25700_regmap_config);
		if (IS_ERR(charger->regmap)) {
			dev_err(&client->dev, "Failed to initialize regmap\n");
			return -EINVAL;
		}

		for (i = 0; i < ARRAY_SIZE(tyzc25700_reg_fields); i++) {
			const struct reg_field *reg_fields = tyzc25700_reg_fields;
		...
		for (i = 0; i < ARRAY_SIZE(tyzc25703_reg_fields); i++) {
			const struct reg_field *reg_fields = tyzc25703_reg_fields;
	...
	charger->chip_id = tyzc25700_field_read(charger, DEVICE_ID);
	...
	if (!dev->platform_data) {
		ret = tyzc25700_fw_probe(charger);
		if (ret < 0) {
	...
	ret = tyzc25700_hw_init(charger);
	if (ret < 0) {
		dev_err(dev, "Cannot initialize the chip.\n");
		return ret;
	}

	tyzc25700_parse_dt(charger);
	tyzc25700_init_sysfs(charger);

	tyzc25700_power_supply_init(charger);
	tyzc25700_init_usb(charger);
	...
	ret = devm_request_threaded_irq(dev, client->irq, NULL,
					tyzc25700_irq_handler_thread,
					irq_flag | IRQF_ONESHOT,
					"tyzc25700_irq", charger);
	if (ret)
		goto irq_fail;
	enable_irq_wake(client->irq);
	tyzc25700_field_write(charger, PROCHOT_PROFILE_IDCHG, 1);
	tyzc25700_field_write(charger, PROCHOT_PROFILE_ACOK, 1);

	tyzc25700_charger = charger;

irq_fail:
	return ret;
}

static void tyzc25700_shutdown(struct i2c_client *client)
{
	int vol_idx;
	struct tyzc25700_device *charger = i2c_get_clientdata(client);
	...
	if (!tyzc25700_field_read(charger, AC_STAT))
		tyzc25700_field_write(charger, EN_LWPWR, 1);
}

#ifdef CONFIG_PM_SLEEP
static int tyzc25700_pm_suspend(struct device *dev)
{
	struct tyzc25700_device *charger = dev_get_drvdata(dev);
...
static int tyzc25700_pm_resume(struct device *dev)
{
	struct tyzc25700_device *charger = dev_get_drvdata(dev);

	tyzc25700_field_write(charger, EN_LWPWR, 0);

	return 0;
}
#endif

static SIMPLE_DEV_PM_OPS(tyzc25700_pm_ops, tyzc25700_pm_suspend, tyzc25700_pm_resume);

static const struct i2c_device_id tyzc25700_i2c_ids[] = {
	{ "tyzc25700"},
	{ },
};
MODULE_DEVICE_TABLE(i2c, tyzc25700_i2c_ids);

#ifdef CONFIG_OF
static const struct of_device_id tyzc25700_of_match[] = {
	{ .compatible = "ti,tyzc25700", },
	{ .compatible = "ti,tyzc25703", },
	{ .compatible = "southchip,sc8885", },
	{ .compatible = "southchip,sc8886", },
	{ },
};
MODULE_DEVICE_TABLE(of, tyzc25700_of_match);
#else
static const struct of_device_id tyzc25700_of_match[] = {
	{ },
};
...
static struct i2c_driver tyzc25700_driver = {
	.probe		= tyzc25700_probe,
	.shutdown	= tyzc25700_shutdown,
	.id_table	= tyzc25700_i2c_ids,
	.driver = {
		.name		= "tyzc25700-charger",
		.pm		= &tyzc25700_pm_ops,
		.of_match_table	= of_match_ptr(tyzc25700_of_match),
	},
};
module_i2c_driver(tyzc25700_driver);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("shengfeixu <xsf@rock-chips.com>");
MODULE_DESCRIPTION("TI tyzc25700 Charger Driver");
